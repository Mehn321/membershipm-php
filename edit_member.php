...
$membershipTypesQuery = "membership_typeID, type, amount FROM membership_types";
$membershipTypesResult = $conn->query($membershipTypesQuery);

if (isset($_GET['id'])) {
    $memberId = $_GET['id'];

    $fetchMemberQuery = "SELECT * FROM members  WHERE members_id = $memberId";
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT m.*, a.city, a.country, a.postcode 
                FROM members m 
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        $getAddressIdSql = "SELECT address_id FROM members  WHERE members_id = ?";
        $getAddressIdStmt = $conn->prepare($getAddressIdSql);
        $getAddressIdStmt->bind_param("i", $member_id);
        $getAddressIdStmt->execute();
        $addressResult = $getAddressIdStmt->get_result();
        $addressData = $addressResult->fetch_assoc();
        $address_id = $addressData['address_id'];
    
        if ($address_id) {
            // Update existing city
            $updateAddressSql = "UPDATE addresses SET city = ?, country = ?, postcode = ? WHERE id = ?";
            $updateAddressStmt = $conn->prepare($updateAddressSql);
            $updateAddressStmt->bind_param("sssi", $city, $country, $postcode, $address_id);
            $updateAddressStmt->execute();
        } else {
            // Create new city
            $newAddressSql = "INSERT INTO addresses (city, country, postcode) VALUES (?, ?, ?)";
            $newAddressStmt = $conn->prepare($newAddressSql);
            $newAddressStmt->bind_param("sss", $city, $country, $postcode);
        
            if ($newAddressStmt->execute()) {
                $address_id = $conn->insert_id;
            
                // Update member with new address_id
                $updateMemberAddressSql = "UPDATE members SET address_id = ? WHERE id = ?";
                $updateMemberAddressStmt = $conn->prepare($updateMemberAddressSql);
                $updateMemberAddressStmt->bind_param("ii", $address_id, $member_id);
                $updateMemberAddressStmt->execute();
            }
        }
                                    ...
                                    <div class="row mt-3">
                                        <div class="col-sm-6">
                                            ...
                                            <input type="text" class="form-control" id="city" name="city"
                                                   placeholder="Enter city" value="<?php echo $memberDetails['city']; ?>" required>
                                        </div>
                                        <div class="col-sm-6">
