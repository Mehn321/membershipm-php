...
$selectQuery = "SELECT m.*, a.city, a.country, a.postcode, t.type as membership_type_name 
        FROM members m 
        LEFT JOIN addresses a ON m.address_id = a.address_id
        LEFT JOIN membership_types t ON m.membership_typeID = t.membership_typeID
        ORDER BY m.members_id DESC";
    ...
    <table id="example1" class="table table-bordered table-striped">
        ...
        <tbody>
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                $membership_typeID = $row['membership_typeID'];
                $membershipTypeQuery = "SELECT type FROM membership_types WHERE membership_typeID = $membership_typeID";
                ...
                if (!empty($row['expiry_date'])) {
                    echo "<a href='memberProfile.php?id={$row['members_id']}' class='btn btn-info'><i class='fas fa-id-card'></i></a>";
                }

                echo "
                    <a href='edit_member.php?id={$row['members_id']}' class='btn btn-primary'><i class='fas fa-edit'></i></a>
                    <button class='btn btn-danger' onclick='deleteMember({$row['members_id']})'><i class='fas fa-trash'></i></button>
                </td>";
                echo "</tr>";
