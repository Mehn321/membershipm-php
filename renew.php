...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $membership_typeID = $_POST['membershipType'];
    $renewDuration = $_POST['extend'];

    $expiryDate = date('Y-m-d', strtotime("+$renewDuration months"));

    $updateMemberQuery = "UPDATE members SET membership_typeID = $membership_typeID, expiry_date = '$expiryDate' WHERE members_id = $memberId";
    $updateMemberResult = $conn->query($updateMemberQuery);

    $totalAmount = $_POST['totalAmount'];
    $renewDate = date('Y-m-d');
    $insertRenewQuery = "INSERT INTO renew (members_id, total_amount, renew_date) VALUES ($memberId, $totalAmount, '$renewDate')";
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="card card-primary">
                            ...
                            <input type="hidden" name="members_id" value="<?php echo $id; ?>">
                                <div class="card-body">
                                    <div class="row">
                                    <div class="col-sm-6">
                                    ...
                                    <select class="form-control" id="membershipType" name="membershipType" required>
                                        ...
                                        if ($membershipTypesResult) {
                                            while ($row = $membershipTypesResult->fetch_assoc()) {
                                                echo "<option value='{$row['membership_typeID']}'>{$row['type']} - {$row['amount']}</option>";
                                            }
                                        } else {
